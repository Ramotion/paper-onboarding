...
class OnboardingContentView: UIView {
    ...
    fileprivate var actionButtonHandlerCallback: (() -> Void)? = nil
...
extension OnboardingContentView {
    ...
    fileprivate func createItem(_ index: Int) -> OnboardingContentViewItem {
        ...
        let item = Init(OnboardingContentViewItem.itemOnView(self)) {
            ...
            if let buttonTitle = info.actionButtonTitle, buttonTitle.isEmpty == false {
                ...
                $0.actionButton?.addTarget(self, action: #selector(OnboardingContentView.onActionButtonTapped(_:)), for: .touchUpInside)
...
extension OnboardingContentView {
    ...
    func onActionButtonTapped(_ button: UIButton) {
