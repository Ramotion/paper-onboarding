...
public struct OnboardingItemInfo {
    ...
    public init(imageName: String, title: String, description: String, iconName: String, color: UIColor, titleColor: UIColor = .white, descriptionColor: UIColor = .white, titleFont: UIFont, descriptionFont: UIFont, actionButtonTitle: String? = nil, actionButtonTextColor: UIColor? = .white, actionButtonBackgroundColor: UIColor? = .clear, actionButtonHandler: (() -> Void)? = nil) {
...
extension PaperOnboarding: GestureControlDelegate {
    ...
    func gestureControlDidTap(tapPoint: CGPoint) {
        guard let cv = self.contentView, let buttonFrame = cv.getCurrentActionButtonFrame(), buttonFrame.contains(tapPoint) else {
            return
        }
